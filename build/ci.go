...
import (
	...
	"factor-mx/crypto/signify"
	"factor-mx/internal/build"
	"factor-mx/params"
	"github.com/cespare/cp"
)

var (
	// Files that end up in the factor*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("factor"),
	}

	// Files that end up in the factor-alltools*.zip archive.
...
func doArchive(cmdline []string) {
	...
	var (
		...
		factor     = "factor-" + basegeth + ext
		alltools = "factor-alltools-" + basegeth + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(factor, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{factor, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with factor-release.pub"
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "factor-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "factor-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "factor.exe" {
			gethTool = file
		} else {
	...
	// first section contains the factor binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.factor.nsi", filepath.Join(*workdir, "factor.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("factor-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "factor.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "factor-mx/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("factor.aar", filepath.Join(GOBIN, "factor.aar"))
		os.Rename("factor-sources.jar", filepath.Join(GOBIN, "factor-sources.jar"))
		return
	}
	...
	archive := "factor-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("factor.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "factor-mx/mobile")
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "factor-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
