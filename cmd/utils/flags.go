...
import (
	...
	"factor-mx/accounts"
	"factor-mx/accounts/keystore"
	"factor-mx/common"
	"factor-mx/common/fdlimit"
	"factor-mx/consensus"
	"factor-mx/consensus/clique"
	"factor-mx/consensus/ethash"
	"factor-mx/core"
	"factor-mx/core/rawdb"
	"factor-mx/core/vm"
	"factor-mx/crypto"
	"factor-mx/eth"
	ethcatalyst "factor-mx/eth/catalyst"
	"factor-mx/eth/downloader"
	"factor-mx/eth/ethconfig"
	"factor-mx/eth/gasprice"
	"factor-mx/eth/tracers"
	"factor-mx/ethdb"
	"factor-mx/ethstats"
	"factor-mx/graphql"
	"factor-mx/internal/ethapi"
	"factor-mx/internal/flags"
	"factor-mx/les"
	lescatalyst "factor-mx/les/catalyst"
	"factor-mx/log"
	"factor-mx/metrics"
	"factor-mx/metrics/exp"
	"factor-mx/metrics/influxdb"
	"factor-mx/miner"
	"factor-mx/node"
	"factor-mx/p2p"
	"factor-mx/p2p/enode"
	"factor-mx/p2p/nat"
	"factor-mx/p2p/netutil"
	"factor-mx/params"
...
var (
	...
	SpaceFlag = cli.BoolFlag{
		Name:  "space",
		Usage: "Space network: factor-mx test network",
	}
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `factor account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "factor.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.GlobalString(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "factor.", tagsMap)
		}
...
// new format.
//
// e.g. factor account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// factor --keystore /tmp/mykeystore --lightkdf account new
//
