...
func TestAccountListEmpty(t *testing.T) {
	factor := runGeth(t, "account", "list")
	factor.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	factor := runGeth(t, "account", "list", "--datadir", datadir)
	defer factor.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	factor := runGeth(t, "account", "new", "--lightkdf")
	defer factor.ExpectExit()
	factor.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	factor := runGeth(t, "--lightkdf", "account", "import", keyfile, "-password", passwordFile)
	defer factor.ExpectExit()
	factor.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	factor := runGeth(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	factor := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer factor.ExpectExit()
	factor.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(factor.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	factor := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(factor.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	factor.SetTemplateFunc("keypath", func(file string) string {
