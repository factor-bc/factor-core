...
`,
	...
	versionCheckCommand = cli.Command{
		...
		Category:  "MISCELLANEOUS COMMANDS",
		Description: `
The version-check command fetches vulnerability-information from https://factor.ethereum.org/docs/vulnerabilities/vulnerabilities.json, 
...
func makecache(ctx *cli.Context) error {
	args := ctx.Args()
	if len(args) != 2 {
		utils.Fatalf(`Usage: factor makecache <block number> <outputdir>`)
	}
...
func makedag(ctx *cli.Context) error {
	args := ctx.Args()
	if len(args) != 2 {
		utils.Fatalf(`Usage: factor makedag <block number> <outputdir>`)
	}
	block, err := strconv.ParseUint(args[0], 0, 64)
	if err != nil {
		utils.Fatalf("Invalid block number: %v", err)
...
You should have received a copy of the GNU General Public License
along with factor. If not, see <http://www.gnu.org/licenses/>.`)
	return nil
}
