...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of factor is "factor". If no
...
func (c *Config) NodeName() string {
	...
	if name == "factor" || name == "factor-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "factor" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by factor 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'factor' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
