...
// Package gethclient provides an RPC client for factor-specific APIs.
package gethclient
...
// Client is a wrapper around rpc.Client that implements factor-specific functionality.
//
...
// GCStats retrieves the current garbage collection stats from a factor node.
...
// MemStats retrieves the current memory stats from a factor node.
...
// GetNodeInfo retrieves the node info of a factor node.
